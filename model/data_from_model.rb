...
class DataFromModel
  ...
  def generate_example_pathways
    ...
    data = data.map do |pathway_data|
      {
        name: pathway_data[0],
        code: convert_float_to_letters(pathway_data[1..32]).join,
        description: wrap(pathway_data[33]),
        wiki: pathway_data[34],
        cost_comparator: (c = pathway_data[35]; c.is_a?(Numeric) ? c : nil )
      }
    end
  end
