  ...
  setUpControls = function() {
    ...
    $("a.choiceLink").on('click touchend', function(event) {
      event.preventDefault();
			$("ul.menu_0 span").html('Your Chosen Pathway');		//added from Indian pathway.js
			$("#pathway_box select").val(0);	//added from Indian pathway.js
      t = $(event.target);
  ...
  switchComparator = function(new_comparator) {
    ...
    if (history['pushState'] != null) {
      history.pushState(choices, codeForChoices(), url());
		...
		if (current_view.switchComparator != null) {					//Added from Indian pathway.js
...
/*    $("a.selectedView").removeClass("selectedView");
	...
	setHelpUrl = function() {
		return $('#help a').attr('href', 'http://www.energy.gov.ng');
  };
...
/*
  updateControls = function(old_choices, choices) {
	...
	//This code from here to below was added from indian pathway.js code 
  ...
  groupclick = function(fraction, groupArr, grouplevel) {
    ...
    if (fraction) {
      ...
      for (m = 0, len2 = groupArr.length; m < len2; m++) {
        choice = groupArr[m];
        if ((choice === 30 || choice === 32) && grouplevel === 4) {
          choices[choice] = 3;
        } else {
          choices[choice] = grouplevel;
        }
  ...
  stopGroupDemo = function(groupChoiceArray) {
	...
	//End of code copied from indian pathway.js code
