...
module Helper
  ...
  def structure
    @structure ||= DataFromModel.new  #ModelStructure.instance   #india code replaced withe ukdecc code
  end
  ...
  def classic_table_row_for_heading(choice_heading, code_id, choiceArr, fraction)
    ...
    if choice_heading == 'Biomass' or choice_heading == 'Bioenergy' or choice_heading == 'Coal' or choice_heading == 'Electricity' or choice_heading == 'Household' or choice_heading == 'Hydropower' or choice_heading == 'Industry' or choice_heading == 'Gas' or choice_heading == 'Nuclear' or choice_heading == 'Services' or choice_heading == 'Solar' or choice_heading == 'Transport' or choice_heading == 'Wind'
      row << "<td width='25px' align='center' id='info-#{code_id}'><a href='/assets/onepage/onepager_Overall/#{choice_heading}.pdf' target='_new' onmouseover='twentyfifty.startGroupDemo(#{groupChoiceArray}); return true;' onmouseout='twentyfifty.stopGroupDemo(#{groupChoiceArray});return true;'><img height='13px' width='13px'  src='/assets/images/get_info.png'></a></td>"
    end
    row_data = ""
    ary ||= Array.new
    for i in 0..3
  ...
  def classic_table_row_for_choice(choice)
    ...
    if choice.number != 33
      row = ["<td class='name' cellpadding='10'><a href='/assets/onepage/#{choice.doc}' target='_new' title='#{choice.descriptions[3]}' onmouseover='twentyfifty.startDemo(#{choice.number}); return true;' onmouseout='twentyfifty.stopDemo(#{choice.number});return true;'>#{choice.name}</a></td>"]
      ...
      row = ["<td class='name' cellpadding='10'><a title='#{choice.descriptions[3]}' onmouseover='twentyfifty.startDemo(#{choice.number}); return true;' onmouseout='twentyfifty.stopDemo(#{choice.number});return true;'>#{choice.name}</a></td>"]

      row << "<td width='20px'></td>"
    end
    choice.levels.each.with_index do |level,i|
      if choice.number == 30 or choice.number == 31 or choice.number == 32 or choice.number == 17
        if level == 1
  ...
  def example_pathways
    @example_pathways ||= DataFromModel.new.example_pathways
  end 
  ...
  def choice_sizes
    sizes = {}
    DataFromModel.new.choices.each do |choice|
